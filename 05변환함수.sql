...
SELECT * FROM EMPLOYEES WHERE SALARY >= '10000'; -- 숫자와 문자 비교에서 자동형변환이 일어나 비교가 가능함
SELECT HIRE_DATE FROM EMPLOYEES WHERE HIRE_DATE = '05/09/30'; -- 날짜와 문자 비교에서 자동형변환이 일어나 비교가 가능함
...
SELECT FIRST_NAME, TO_CHAR(HIRE_DATE, 'YY-MM-DD AM') FROM EMPLOYEES; -- AM, PM 둘다 오전 오후 각각의 시간에 맞게 표기되어 출력됨 (둘다 같은 결과값)
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YY-MM-DD HH24:MI:SS') FROM DUAL; -- HH24 를 쓰면 24시간 형식으로 표기해줌
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YY-MM-DD PM') FROM DUAL;
...
SELECT TO_NUMBER('2014','9999999') FROM DUAL;
...
SELECT '$2,000' + 2000 FROM DUAL; --실행X 자동 형변환이 되지만 문자에 기호가 붙어있으면 자동형변환이 되지 않아서 수동 형변환을 해서 형식을 맞춰줘야함
SELECT TO_NUMBER('$2,000', '$9,999') FROM DUAL; --숫자로 형변환되면서 기호들이 제거되고 숫자만 남게 됨
...
SELECT TO_DATE('2024-02-07', 'YYYY-MM-DD') - HIRE_DATE FROM EMPLOYEES; -- 차이나는 일자를 표기
...
-- DECODE(값, 비교값, 결과값, ....., ELSE문) -> 자바의 IF문과 같은 역할
...
-- CASE ~ WHEN ~ THEN ~ ELSE ~ END -> 자바의 SWICH문과 같은 역할
SELECT FIRST_NAME
       ,JOB_ID
       ,SALARY
