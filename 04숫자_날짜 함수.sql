-- 숫자함수
-- ROUND(반올림할 문자, 반올림 할 숫자의 위치 -> 오른쪽기준 / 0이면 정수의 첫째자리 반올림) - 반올림
...
-- 날짜 반올림 ROUND, 절삭 TRUNK
SELECT SYSDATE, ROUND(SYSDATE), TRUNC(SYSDATE) FROM DUAL; -- 일 기준으로 반올림, 절삭


SELECT TRUNC(SYSDATE, 'YEAR') FROM DUAL; -- 년 기준으로 절삭 (절삭된 데이터는 01/01일로 표기)
SELECT TRUNC(SYSDATE, 'MONTH') FROM DUAL; -- 월 기준으로 절삭 (절삭된 데이터는 01일로 표기)
